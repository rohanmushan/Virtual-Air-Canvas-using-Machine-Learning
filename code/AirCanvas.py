...
current_color = (0, 0, 255)  # Default red (BGR format)
current_brush_size = 2  # Default brush size

# Dictionary to store points for each color
color_points = defaultdict(lambda: deque(maxlen=deque_len))
cooldown_time = 1
last_button_time = time.time()

# Canvas - we'll maintain two versions:
# - persistent_canvas: keeps all drawings
# - temp_canvas: for current frame display
...
def save_canvas():
    ...
    cv2.imwrite(filename, persistent_canvas)  # Save the persistent canvas
...
def main():
    ...
    while True:
        ...
        # Start with fresh frame and buttons
        temp_canvas = persistent_canvas.copy()
        draw_buttons(frame)
            ...
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                if smoothed_y < 65:
                    ...
                    if now - last_button_time > cooldown_time:
                        if 20 <= smoothed_x <= 140:  # Clear button
                            color_points.clear()
                            persistent_canvas.fill(255)
                        elif 160 <= smoothed_x <= 260:  # Color button
                            new_color = show_color_picker(current_color)
                            if new_color:
                                current_color = new_color
                        elif 500 <= smoothed_x <= 620:  # Save button
                            save_canvas()
        ...
        # Draw all lines for all colors on both canvases
